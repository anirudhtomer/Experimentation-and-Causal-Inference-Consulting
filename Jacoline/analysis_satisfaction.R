...
patient_data = read.spss(file = "Datasetlongformat_satisfaction.sav", to.data.frame = T)
...
patient_data = patient_data[complete.cases(patient_data[, c("Patient_satisfaction", "sex", "joint", "age", "BMI", "Com_min2", "Treatment",
                                                             "Score_Paindetect")]),]

patient_data$Patient_satisfaction = as.numeric(as.character(patient_data$Patient_satisfaction))

full_model = lme(data=patient_data,
                 fixed = Patient_satisfaction~(BMI + age + sex + joint + Treatment + Com_min2 + Score_Paindetect) * Time, 
...
ggplot(data=plotData) + geom_line(aes(x=Time, y=pred, 
  ...
  theme_bw() + 
  xlab("Time (weeks)") + ylab("Patient satisfaction (Com_min2 = 'ja')")
